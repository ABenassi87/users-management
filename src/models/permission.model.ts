...
import { IUserPermission } from './linking-models/user-permission.model';
...
export interface IPermissionModel extends Model<IPermission> {
  getEffectivePermissions(username: string): Promise<IPermission[]>;
  getScope(username: string): Promise<string[]>;
}
...
schema.statics = {
   ...
   */
  getEffectivePermissions: async (user: IUser): Promise<IUserPermission[]> => {
    return new Promise<IUserPermission[]>((resolve, reject) => {
      try {
        const User = mongoose.model('user');

        user = await RestHapi.find(User, user._id, { $embed: ['permissions', 'role.permissions', 'groups.permissions'] }, Log);
        ...
        user.groups.forEach(function(group) {
          group.group.permissions.forEach(function(groupPermission) {
            ...
            permissions.find(function(permission, index) {
              if (permission.permission._id.toString() === groupPermission.permission._id.toString()) {
                matchIndex = index;
                return true;
              }
            });
   ...
   */
  getSpecificScope: async (payload: IUser): Promise<any[]> => {
    return new Promise<any[]>(async (resolve, reject) => {
      try {
        let user = null;

        await User.findById(payload._id, (err, res: IUser) => {
          if (err) reject(err);
          user = res;
        });
...
export const Permission = mongoose.model<IPermission>('permission', schema) as IPermissionModel;
